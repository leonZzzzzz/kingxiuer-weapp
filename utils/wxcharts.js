...
function drawPointShape(points, color, shape, context) {
  context.beginPath();
  context.setStrokeStyle("#eb94a9");
  context.setLineDash([1, 0]);
  context.setFillStyle('#eb94a9');
  if (shape === 'diamond') {
    points.forEach(function (item, index) {
      if (item !== null) {
...
function drawPointText(points, series, config, context) {
  ...
  points.forEach(function (item, index) {
    if (item !== null) {
      ...
      context.fillText(formatVal, item.x - measureText(formatVal) / 2, item.y - 8);
    }
  });
  context.closePath();
  context.stroke();
...
function drawAreaDataPoints(series, opts, config, context) {
  ...
  series.forEach(function (eachSeries, seriesIndex) {
    ...
    splitPointList.forEach(function (points) {
      ...
      // 绘制区域数据 area
      context.beginPath();
      const grd = context.createLinearGradient(0, 0, 0, 300)
      grd.addColorStop(0, '#eb94a9')
    ...
    if (opts.dataPointShape !== false) {
      var shape = config.dataPointShape[1];
